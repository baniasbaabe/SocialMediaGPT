templatizing_prompt = """
    ...
    My [personal consequence or change].\nI [action taken] of my [possessions or attachments].'''.\n\
    Now, I will give you a LinkedIn posts. I want you to generate only the reusable template. The template should be generic and used on any topic.\
    The template should use the same formatting, that means the same spaces and enters.\
    I want it to look less like of a post but more like a template anyone could use. The output should have the following \
    dictionary format in minimized form (no spaces, ideally one line): {{"title": '''Short title of template''', "post": '''The template you made'''}}\
...
creating_posts_prompt = """
    You are a viral Content creator. You will take a template and a topic, and generate posts from it.\
    Here is the template between '''''': \
    '''{TEMPLATE}''' \
    Based on this template, generate {NUMBER_OF_POSTS} different posts where you will only fill in the brackets.\
